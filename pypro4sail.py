...
def run_TIR(emisVeg, emisSoil, T_Veg, T_Soil, LAI, hot_spot, solar_zenith, solar_azimuth, view_zenith, view_azimuth,
            LIDF, T_VegSunlit=None, T_SoilSunlit=None, T_atm=0, wav=None):
    ...
    if type(LIDF) == tuple or type(LIDF) == list:
        if len(LIDF) != 2:
            ...
            lidf = four_sail.calc_lidf(LIDF[0], LIDF[1])
    else:
        lidf = four_sail.calc_lidf_campbell(LIDF)
    ...
    if wav:
        Hvh = PlanckFunction(wav, T_VegSunlit)
        Hvc = PlanckFunction(wav, T_Veg)
        Hgh = PlanckFunction(wav, T_SoilSunlit)
        Hgc = PlanckFunction(wav, T_Soil)
        Hsky = PlanckFunction(wav, T_atm)
    else:
    ...
    Lw = rdot * Hsky + (
                aeev * Hvc + gammasot * emisVeg * (Hvh - Hvc) + aees * Hgc + tso * emisSoil * (Hgh - Hgc))

    if wav:
        TB_obs = InvPlanckFunction(wav, Lw)
    else:
        TB_obs = (Lw / SB) ** (0.25)
...
C1 = 1.191 * pow(10, 8)
C2 = 14388
def PlanckFunction(wave, T):
    '''
    Calculate the radiance at single-wavelength with Planck Function
    '''
    numerator = C1 * pow(wave, -5)
    denominator = np.exp(C2 / (wave * T)) - 1
    radiance = numerator / denominator
    return radiance

def InvPlanckFunction(wave, Rad):
    c = np.log(C1 * pow(wave, -5) / Rad + 1)
    T = C2 / (wave * c)
    return T